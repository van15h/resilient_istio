...
def filter():
    ...
    if len(request.args) == 0:
        ...
        return Response(json.dumps(open_file()), status=200)
...
def filter_alerts(obj):
    ...
    if 'aggregate' in obj:
        if (str(obj.get('aggregate')) == 'count'):
            ...
            return Response('should be agreggate=count', status=400)
    data = open_file()
    response = []
    number_of_persons = 0
    if aggr == '':
        ...
        for alert in data:
            if t1 <= datetime.strptime(alert['timestamp'], '%Y-%m-%dT%H:%M:%S.%f%z') <= t2:
...
def delete(id):
    """delete alert by id"""
    data = open_file()
    app.logger.debug('total of alerts: ' + str(len(data)))
    # process request
...
def get_details(id):
    ...
    for alert in data:
        if str(alert['id']) == str(id):
            app.logger.debug('id found')
            return Response(json.dumps(alert, indent=2), status=200)
    return Response('', status=404)
